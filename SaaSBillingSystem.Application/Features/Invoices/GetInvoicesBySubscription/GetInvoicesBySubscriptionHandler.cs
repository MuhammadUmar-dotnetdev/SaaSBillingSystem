...
{
    public class GetInvoicesBySubscriptionHandler: IRequestHandler<GetInvoicesBySubscriptionCommand, Result<List<InvoiceResponse>>>
    {
        private readonly IInvoiceRepository _invoiceRepository;
        private readonly IOrganizationRepository _organizationRepository;
        private readonly ISubscriptionRepository _subscriptionRepository;
        public readonly IPlanRepository _planRepository;
        public GetInvoicesBySubscriptionHandler(IInvoiceRepository invoiceRepository, IOrganizationRepository organizationRepository, ISubscriptionRepository subscriptionRepository, IPlanRepository planRepository)
        {
        ...
        public async Task<Result<List<InvoiceResponse>>> Handle(GetInvoicesBySubscriptionCommand command, CancellationToken cancellationToken)
        {
            var invoicesList = await _invoiceRepository.GetBySubscriptionAsync(command.SubscriptionId);
            if (invoicesList.Count == 0)
            {
                return Result<List<InvoiceResponse>>.Failure($"Invoice list is empty for id {command.SubscriptionId}");
            }
